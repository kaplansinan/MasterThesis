...
def load_local_data(path):
    paths = glob.glob(os.path.join(path, "*.jpeg"))
...
#
#
# vae.save_weights('vae_diabet_32_300.h5')
...
grid_x = np.linspace(50, 100, 25)
grid_y = np.linspace(50, 100, 25)
...
for i, xi in enumerate(grid_x):
    for j, yi in enumerate(grid_y):
        ...
        ax.set_axis_off()
        fig.add_axes(ax)
        ax.imshow(face, aspect='normal')
        fig.savefig('GeneratedImages_VAE_128/ret_%s_%s.png' %(i,j), dpi=1)
# file_path_to_save_img+'retina_%s.png' % (img),

# z_sample = np.array([[0.01, 0.5]])
# x_decoded = generator.predict(z_sample)
#
# # x_decoded = x_decoded
